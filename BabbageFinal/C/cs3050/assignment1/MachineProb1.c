...
/* Function Purpose: Performs a BFS walk of adj. list
 * Input params: Vertex* adjList[] - adjacency list head pointer, Vertex* listForWalk[] - list of vertices to check for simple graph
 * Return param: int vertCount - number of vertices that have been visited by walk (equal to total # of verts in graph for cycle)
 **/
int walkBFS(Vertex* adjList[], Vertex* listForWalk[]){
	int goOn = TRUE;
	int vertCount = 1;
