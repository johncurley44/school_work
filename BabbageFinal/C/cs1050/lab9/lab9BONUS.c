...
//Function Prototypes
 ...
 void suffixSearch(char* stringArray[MAX], char* searchSuffix);
 ...
 **/
 int main(void){
	//variable declaration
	 ...
	 for(m = 0; m<NAMES; m++){
		printf("\nSearch for suffix %d: ", m+1);
		scanf("%s", tempSearch);
		suffixSearch(nameArray, tempSearch);
	 }
	 printf("\nProgram Terminated.\n\n");
...
void suffixSearch(char* stringArray[MAX], char* searchSuffix){
	int len = ARRAYSIZE;
	int lens = length(searchSuffix);
	int countBack = lens;
	int count1, notHere = 0, found = 0;
	//Iterates through each string in string array
	for(count1 = 0; count1<len; count1++){
		//Iterates through each character in searchPrefix
		for(countBack = lens; countBack>(len-lens); countBack--){
			//If characters in respective locations of searchPrefix and current string
			// from string array do not match, sets notHere = 1
			if(*(stringArray[count1]+countBack-1) != *(searchSuffix + countBack-1)){
				notHere = 1;
			}
		}
		//Prints that the prefix was found at current location, found=1 indicates 
		// that the prefix was found in at least one location
		if(notHere == 0){
			printf("Suffix %s was found at index %d\n", searchSuffix, count1);
			found = 1;
		}
		//Resets notHere
		notHere = 0;
	...
	//Prints statement that prefix wasn't found
	if(found == 0){
		printf("Suffix %s was not found in the array\n", searchSuffix);
	}
}
